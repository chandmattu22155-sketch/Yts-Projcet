...
async function fetchData(filters = {}, page = 1) {
  ...
  try {
   ...
   let query_params = `page=${page}&limit=${filters.limit || 10}`;
    ...
    if (filters.search) {
      query_params += `&query_term=${filters.search}`
    }
    ...
    if (filters.genre) {
      query_params += `&genre=${filters.genre}`;
    }

    if (filters.minimum_rating) {
      query_params += `&minimum_rating=${filters.minimum_rating}`
    }

    if (filters.sort_by) {
      query_params += `&sort_by=${filters.sort_by}`
    }
    if (filters.order_by) {
    ...
    console.log(error);
    movieContainer.innerHTML =
      "<p class='text-white text-center'>Something went wrong</p>";
  } finally {
    hideLoader();
  }
}
...
function onClick() {
  ...
  const minimum_rating = selectMinimumRating.value;
  const sort_by = selectSortBy.value;
  const order_by = selectOrderBy.value;
